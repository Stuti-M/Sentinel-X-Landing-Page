...
export function SentinelCommandCenter() {
  ...
  return (
    <main className="console-grid min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="top" className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-[1600px] items-center px-4 py-16 md:px-8 lg:py-24">
      ...
      <section id="command" className="border-y border-border bg-background/65 px-4 py-20 md:px-8">
          ...
          <div className="mt-8 grid gap-4 xl:grid-cols-[.9fr_1.45fr_1fr]">
            <Panel>
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-2 gap-2">{PRESETS.map((preset) => <button key={preset} type="button" disabled={runState === "running"} onClick={() => setScenario(`${preset} reported near a protected ecological zone. Verify the incident, assess the risk, and recommend an immediate response.`)} className="min-h-12 rounded-sm border border-border bg-background/30 px-3 py-2 text-left font-mono text-[9px] text-muted-foreground transition-colors hover:border-primary/50 hover:text-primary disabled:opacity-50">{preset}</button>)}</div>
            ...
            <div className="space-y-4">
              <Panel>
                ...
                <div className="p-4"><Workflow activeNode={activeNode} running={runState === "running"} /><div className="mt-5 border-t border-border pt-4"><div className="flex items-center gap-3"><span className={cn("grid size-7 shrink-0 place-items-center rounded-full border", runState === "running" ? "border-primary text-primary" : runState === "complete" ? "border-eco text-eco" : "border-border text-muted-foreground")} >{runState === "complete" ? <Check className="size-3.5" /> : <Activity className={cn("size-3.5", runState === "running" && "animate-pulse")} />}</span><div className="min-w-0"><p className="truncate font-mono text-[10px] text-foreground">{stage >= 0 ? STAGES[stage].label : "AWAITING MISSION"}</p><p className="mt-1 font-mono text-[8px] text-muted-foreground">{runState === "running" ? `STEP ${stage + 1} / ${STAGES.length}` : runState === "complete" ? "MISSION COMPLETE" : "SYSTEM READY"}</p></div></div><div className="mt-3 h-0.5 overflow-hidden bg-muted"><div className="h-full bg-primary transition-all duration-700" style={{ width: `${stage < 0 ? 0 : ((stage + 1) / STAGES.length) * 100}%` }} /></div></div></div>
              </Panel>
      ...
      ].map((agent) => { const Icon = agent.icon; return <Panel key={agent.name} className="group transition-colors hover:border-primary/40"><div className="p-6 md:p-8"><div className="grid grid-cols-[minmax(0,1fr)_auto] gap-5"><div className="min-w-0"><p className="font-mono text-[9px] text-muted-foreground">{agent.code}</p><h3 className="mt-3 truncate font-mono text-xl font-medium">{agent.name}</h3><p className={cn("mt-2 font-mono text-xs", agent.color)}>{agent.quote}</p></div><div className="grid size-12 shrink-0 place-items-center border border-border bg-background/50"><Icon className={cn("size-5", agent.color)} /></div></div><div className="my-6 h-px bg-border" /><div className="grid gap-6 sm:grid-cols-2"><div><p className="mb-3 font-mono text-[9px] text-muted-foreground">DIRECTIVES</p>{agent.roles.map((role) => <p key={role} className="mb-2 flex gap-2 text-xs leading-5"><ChevronRight className={cn("mt-1 size-3 shrink-0", agent.color)} />{role}</p>)}</div><div><p className="mb-3 font-mono text-[9px] text-muted-foreground">CAPABILITIES</p><div className="flex flex-wrap gap-2">{agent.tools.map((tool) => <span key={tool} className="rounded-sm border border-border bg-background/40 px-2 py-1.5 font-mono text-[9px] text-muted-foreground">{tool}</span>)}</div></div></div></div></Panel> })}</div></div></section>

      <section id="orchestration" className="px-4 py-20 md:px-8"><div className="mx-auto max-w-[1600px]"><SectionLabel index="04">Orchestration Layer</SectionLabel><div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr] lg:items-center"><div><h2 className="text-3xl font-semibold sm:text-4xl">One signal.<br />A coordinated response.</h2><p className="mt-5 max-w-md text-sm leading-7 text-muted-foreground">n8n routes each crisis through specialized reasoning stages while preserving an inspectable chain of decisions.</p><div className="mt-6 flex items-center gap-2 font-mono text-[10px] text-foreground"><Network className="size-4 text-warning" />POWERED BY n8n</div></div><Panel><PanelHeader icon={Network} title="Live Node Graph" meta="AUTONOMOUS PIPELINE" /><div className="p-6"><div className="grid gap-3 sm:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] sm:items-center">{["WEBHOOK","FIELD RESEARCHER","POLICY DIRECTOR","RESPONSE"].map((node,i) => <div key={node} className="contents"><div className="rounded-sm border border-border bg-background/50 p-4 text-center font-mono text-[9px]"><CircleDot className={cn("mx-auto mb-3 size-5", i === 0 ? "text-warning" : i === 3 ? "text-eco" : "text-primary")} />{node}</div>{i<3 && <ArrowRight className="mx-auto hidden size-4 text-muted-foreground sm:block" />}</div>)}</div></div></Panel></div></div></section>

      <section id="architecture" className="border-y border-border bg-panel/30 px-4 py-20 md:px-8"><div className="mx-auto max-w-[1600px]"><SectionLabel index="05">System Architecture</SectionLabel><h2 className="text-3xl font-semibold sm:text-4xl">Transparent by design.</h2><div className="mt-8 grid grid-cols-2 gap-px overflow-hidden rounded-md border border-border bg-border md:grid-cols-6">{[[Globe2,"BROWSER"],[ShieldCheck,"NODE.JS PROXY"],[Network,"n8n WEBHOOK"],[Bot,"AGENTS"],[Braces,"STRUCTURED JSON"],[Eye,"BROWSER"]].map(([Icon,label],i) => { const I = Icon as typeof Globe2; return <div key={label as string} className="relative bg-panel p-5 text-center"><I className="mx-auto mb-3 size-5 text-primary" /><p className="font-mono text-[9px]">{label as string}</p>{i<5 && <ChevronRight className="absolute -right-3 top-1/2 z-10 hidden size-5 -translate-y-1/2 bg-background text-muted-foreground md:block" />}</div>})}</div><p className="mt-4 font-mono text-[9px] text-muted-foreground">NO CLIENT-SIDE KEYS · SERVER-MEDIATED REQUESTS · TRACEABLE OUTPUT</p></div></section>

      <section id="impact" className="px-4 py-20 md:px-8"><div className="mx-auto max-w-[1600px]"><SectionLabel index="06">Impact & Ethics</SectionLabel><div className="grid gap-10 lg:grid-cols-2"><div><h2 className="max-w-xl text-3xl font-semibold leading-tight sm:text-4xl">Autonomy with accountability.</h2><blockquote className="mt-7 max-w-xl border-l-2 border-primary pl-5 text-lg leading-8 text-foreground">“Automation should accelerate environmental response — not remove human accountability.”</blockquote><div className="mt-8 grid grid-cols-2 gap-3">{["Evidence-backed reasoning","Transparent agent workflow","Structured outputs","Human oversight","Traceable decisions"].map((item) => <div key={item} className="flex items-center gap-2 border-b border-border py-3 text-xs"><Check className="size-3.5 shrink-0 text-eco" />{item}</div>)}</div></div><div className="grid gap-4 sm:grid-cols-2"><Panel><div className="p-6"><p className="font-mono text-4xl text-primary">13</p><Trees className="my-8 size-7 text-primary" /><p className="font-mono text-xs">SDG 13</p><p className="mt-2 text-sm text-muted-foreground">Climate Action</p></div></Panel><Panel><div className="p-6"><p className="font-mono text-4xl text-eco">15</p><Globe2 className="my-8 size-7 text-eco" /><p className="font-mono text-xs">SDG 15</p><p className="mt-2 text-sm text-muted-foreground">Life on Land</p></div></Panel></div></div></div></section>
